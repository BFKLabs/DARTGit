classdef AxesContextMenuGit < handle
    ...
    methods
        % class constructor
        function obj = AxesContextMenuGit(hFig,hAx,mStr,useCheck)
            ...
            % sets the use check flag
            if exist('useCheck','var')
                obj.useCheck = useCheck;
            else
                obj.useCheck = false;
            end
        ...
        function updateMenuLabels(obj,mStrNw)
            ...
            for i = 1:obj.nLbl
                ...
                resetObjPos(obj.hLbl{i,2},'Width',obj.tWid);
            end
...
%             pPos = get(get(obj.hMenu,'Parent'),'Position');            
            ...
            x0 = pNw(1) - pPos(1);
            if (x0 + mnPos(3) + xOfs) > pPos(3)
                x0 = x0 - mnPos(3);
            end
            
            % sets the menu bottom position 
            y0 = pNw(2) - (pPos(2) + mnPos(4) + yOfs);
            if y0 < 0
                y0 = y0 + mnPos(4);
            end
    ...
    methods (Static)
        ...
        function menuSelect(hMenu,~,obj)
            ...
            % updates the menu check mark (if required)
            if obj.useCheck
